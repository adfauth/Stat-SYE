...
*Note: all of this exploration is with the un-filtered data. For any graphs using the entire data set, those include misidentifications. Without really knowing whether these identifications are of mis-identified native species or abiotic sounds, we don't know how this is effecting the vizualizations* LOOK AT SOME MORE AUDIO
...
Using R we started exploring the data. We recorded for `{r} basic_stats$duration` resulting in `{r} basic_stats$count` total detections and `{r} basic_stats$sr` total species. Figure 2 depicts the top ten most common species, the most common being Spring Peepers (*Pseudacris crucifer*) with `{r} common_species$species[1]` total detections. Table 2 shows the sites with the most detections and species -- a site in Donnerville State Forest had the most detections with `{r} site_stats$count[1]`, but the site with the highest species richness was on St. Lawrence University campus with `{r} max(site_stats$species_richness)` species identified.

```{r}
# NOTE: we still have erroneous species!
# figure 2:
