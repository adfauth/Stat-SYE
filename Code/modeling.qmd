...
all_sites <- merged_df |> filter(as_date(date_time) == "2024-04-08") |> 
    filter(between(time, time_start, time_end)) |>
    group_by(id) |> # want to complete the gaps for each site/AM
    complete(time = as_hms(seq(as.numeric(time_start), as.numeric(time_end), by = 3)), 
    fill = list(value = NA)) |> 
    ungroup() |> 
    mutate(Presence = if_else(!is.na(probability),
...
# 3. Fit the model
all_sites_mod <- gamm(Presence ~ s(time_num) + s(deployment_num, bs = "re"), 
                   family = binomial,
                   data = all_sites,
...
This needs work... something is not working right
