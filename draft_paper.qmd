...
Using R, we started exploring the data. We recorded for `{r} basic_stats$duration` resulting in `{r} basic_stats$count` total detections and `{r} basic_stats$sr` total species. The most common were Spring Peepers (*Pseudacris crucifer*) with `{r} common_species$count[1]` total detections. A site in Donnerville State Forest had the most detections with `{r} site_stats$count[1]`, but the site with the highest species richness was on St. Lawrence University campus with `{r} max(site_stats$species_richness)` species identified.
